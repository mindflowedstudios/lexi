...
const getInitialLanguage = (): string => {
  const saved = localStorage.getItem("lexios_language");
  const isInitialized = localStorage.getItem("lexios_language_initialized");
  ...
  if (!isInitialized) {
    ...
    localStorage.setItem("lexios_language", detectedLanguage);
    localStorage.setItem("lexios_language_initialized", "true");
...
i18n
  ...
  .init({
    ...
    detection: {
      ...
      lookupLocalStorage: "lexios_language",
      caches: ["localStorage"],
    },
  });
