...
const handlePreloadError = (event: Event) => {
  console.warn("[LexiOS] Chunk load failed:", event);

  // Don't reload if offline - it won't help and will cause a flash loop
  if (!navigator.onLine) {
    console.warn("[LexiOS] Skipping reload - device is offline");
    return;
  }

  // Use the same loop protection as index.html's stale bundle detection
  const reloadKey = "lexios-stale-reload";
  const lastReload = sessionStorage.getItem(reloadKey);
  const now = Date.now();

  // If we reloaded in the last 10 seconds, don't reload again
  if (lastReload && now - parseInt(lastReload, 10) < 10000) {
    console.warn("[LexiOS] Recently reloaded for stale bundle, skipping to prevent loop");
    return;
  }

  // Mark that we're reloading
  sessionStorage.setItem(reloadKey, String(now));
  console.log("[LexiOS] Reloading for fresh assets...");
